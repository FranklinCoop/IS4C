...
if (!class_exists('FpdfWithBarcode')) {
  ...
  class FCC_Tiny_PDF extends FpdfWithBarcode
  {
  ...
  function FCC_Tiny($data,$offset=0){
    global $FANNIE_OP_DB;
    global $FANNIE_ROOT;
    ...
    $pdf=new FCC_Tiny_PDF('P', 'mm', 'Letter');
    ...
    */
        ...
        */
        ...
        $pdf->SetXY($genLeft+1 - 3, $unitTop+16.2);
        $pdf->Cell(24,4,$size,0,0,'R');
        $pdf->SetXY($genLeft+25, $unitTop+16.2);
        //please use the order  "Local, Organic, NONGMO, Gluten Free
        if ($showLocal) {$pdf->Image($FANNIE_ROOT.'src/images/Local.jpg',$genLeft+26 -3,$unitTop+16,3);}
        if ($showOrganic) {$pdf->Image($FANNIE_ROOT.'src/images/Organic.jpg',$genLeft+29.5 -3,$unitTop+16,3);}
        if ($showNONGMO) {$pdf->Image($FANNIE_ROOT.'src/images/non-gmo.jpg',$genLeft+33 -3,$unitTop+16,3);}
        if ($showGlutenFree) {$pdf->Image($FANNIE_ROOT.'src/images/Gluten-Free.jpg',$genLeft+36.5 -3,$unitTop+16,3);}        
        ...
        $pdf->SetXY($genLeft+28-.5 -3, $unitTop+28.5);
        ...
        */
        ...
        if (strlen($upc) <= 11)
            $pdf->UPC_A($genLeft+3.5 -1.5, $unitTop+21.5,$upc,3);
        else
            $pdf->EAN13($genLeft+3.5 -1.5, $unitTop+21.5,$upc,3);
