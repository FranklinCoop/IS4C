...
class VPBPIII extends FannieRESTfulPage
{
...
var btnChangeSelected = function(direction)
{
    ...
    document.getElementById('cur-target-margin').innerHTML = uniqTable.rows[currentRow].cells[0].dataset.target;
...
JAVASCRIPT;
    ...
    public function getTable()
    {
        ...
        while ($row = $dbc->fetch_row($result)) {
            ...
            $td .= sprintf("<tr id=row%s class='%s %s'>
                <td class=\"sub\" 
                    data-upc=\"%s\"
                    data-cost=\"%s\" 
                    data-margin=\"%.2f\"
