...
class FCC_MergeOldTransArchiveTask extends FannieTask
{
    ...
    private function getDates($sql,$table)
    {
        ...
        try {
            ...
            */
            $this->cronMsg('Failed to find dates in '.$table.'. Details: '
        ...
        /* get table definition in partitioning mode to
           have a list of existing partitions */
            ...
            if ($FANNIE_ARCHIVE_METHOD == "partitions") {
                ...
                $query = "CREATE TABLE $table LIKE $FANNIE_TRANS_DB.".$table;
                if ($sql->dbmsName() == 'mssql') {
                    $query = "SELECT * INTO $table FROM $FANNIE_TRANS_DB.dbo.".$table."
                ...
                $query = "INSERT INTO " . $table . "
                          SELECT * FROM " . $FANNIE_ARCHIVE_DB . $sql->sep() . $table."
                          WHERE ".$sql->datediff('datetime', "'$date'")."= 0";
                try {
