...
class FCCMarginReport extends FannieReportPage 
{
    public $description = '[Franklin Community Coop Margin] (Achived Margin) lists item movement with margin information.';
    public $report_set = 'Movement Reports';

    protected $title = "Fannie : FCC Margin Movement Report";
    protected $header = "FCC Margin Movement Report";

    protected $sort_column = 5;
    protected $sort_direction = 1;
    //UPC, Brand, Dec, Qty, $, Dept#, Department, cost, retail and margin,contribution
    protected $report_headers = array('UPC', 'Brand', 'Desc', 'Dept#', 'Dept','qty', 'cost', 'retail', 'Margin','Markup', 'Contrib');
    ...
    public function fetch_report_data()
    {
        ...
        $query = "SELECT d.upc,
                    ...
                    SUM(CASE WHEN (d.cost > 0 AND d.total < 0) OR (d.cost < 0 AND d.total > 0) THEN -1*d.cost ELSE d.cost END) AS cost, 
                    SUM(CASE WHEN (d.cost > 0 AND d.total < 0) OR (d.cost < 0 AND d.total > 0) THEN -1*d.total ELSE d.total END) AS total
                  FROM $dlog AS d "
        ...
        for ($i=0; $i<count($data); $i++) {
            ...
            $contrib = 0;
            if (is_numeric($data[$i][8]) && is_numeric($data[$i][7])) {
                $contrib = ($data[$i][8] - $data[$i][7]) / $sum_total * 100;
            }
    ...
    private function rowToRecord($row)
    {
        ...
        return array(
            ...
            $row['dept_name'],
            $row['qty'],
            sprintf('%.3f', $row['cost']/$row['qty']),
            sprintf('%.2f', $row['total']/$row['qty']),
            sprintf('%.2f', $margin),
