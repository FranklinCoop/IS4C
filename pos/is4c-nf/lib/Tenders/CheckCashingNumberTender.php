...
/**
  @class CheckCashingNumberTender
  Tender modlue that uses member numbers as a postive check file.
  Only members can cash checks.
*/
class CheckCashingNumberTender extends TenderModule 
{
    /**
      Check for errors
    ...
    public function errorCheck()
    {
        ...
        if ( CoreLocal::get("isMember") ==0) {
            return DisplayLib::boxMsg(
                _("Only Registered Accounts May Cash Checks"),
                false,
                $clearButton
            );
        } else {
            $dbc = Database::pDataConnect();
            $query = sprintf("SELECT WriteChecks FROM custdata 
                    WHERE CardNo=%d", 
