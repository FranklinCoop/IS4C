...
use COREPOS\pos\lib\LocalStorage\CoreLocal;
//use \Exception;

class FoodBankCouponParser extends Parser implements AdminLoginInterface
{
    private $scaleprice = 0;

    public function check($str)
    {
        if (substr($str,0,2) == "FB" && strlen($str) <= 15) {
            return true;
        }
    ...
    public function parse($str)
    {
        ...
        try {
            ...
            if (strlen($str) > 2) {
                //echo "let's go";
                $row = $this->validationCheck(substr($str,2));
    ...
    private function buyCoupon ($upc, $plu, $json) {
        ...
        try {
            ...
            $couponRet = $dbc->query($queryCoupon);
            $coupon = $dbc->fetchRow($couponRet);

            $queryItem = "SELECT unitPrice,regPrice,total, memDiscount,trans_type,trans_subtype,tax,cost,volDiscType,
                        volume,volSpecial,mixMatch,matched, scale
                FROM localtemptrans WHERE upc ={$upc} AND trans_type != 'L' GROUP BY upc";
            $dbc = Database::tDataConnect();
            $itemRet = $dbc->query($queryItem);
            $item = $dbc->fetchRow($itemRet);
            echo $item['unitPrice'];
            ...
            TransRecord::addRecord(array(
                ...
                'trans_subtype' => $item["trans_subtype"], 
                'trans_status' => '', 
                'department' => $coupon["department"], 
                'quantity' => 1, 
                ...
                'ItemQtty' => 1, 
                'volDiscType' => $item["volDiscType"],
                'volume' => $item["volume"],
                'VolSpecial' => $item["volSpecial"], 
                'mixMatch' => $item["mixMatch"], 
    ...
    private function validationCheck ($upc) {
        $dbc = Database::pDataConnect();

        $query = "SELECT * FROM FoodBankCoupon where item_upc = ".$upc;
